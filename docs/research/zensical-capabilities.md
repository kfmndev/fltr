...
- Two built-in schemes: `default` (light) and `slate` (dark) — the Material equivalents.
...
Root `zensical.toml` + `docs/` docs_dir + `.github/workflows/docs.yml` — matches the ticket's assumption. `docs_dir` and `site_dir` are configurable ([setup/basics](https://zensical.org/docs/setup/basics/)); `docs_dir` cannot be `.` (temporary limitation, backlog #101). `site_name` is the only required setting. Zensical also still reads `mkdocs.yml` if you'd rather not migrate yet.
...
- The roadmap lists an **"MkDocs `exclude` plugin replacement"** under "Up next" (https://zensical.org/roadmap/#mkdocs-compatibility) — i.e. planned, not shipped.
...
- Native versioning is on the roadmap (https://zensical.org/roadmap/#versioning — git-based versions, folder-based versions, selective publishing, focused rebuilds) with no delivery date; the mike fork is maintained "until Zensical provides native versioning support".
...
1. Whether `important`/`caution` will become first-class admonition types (or whether a type-aliasing option will be added to `pymdownx.quotes`) — not documented anywhere; would need a backlog search or asking the team.
...
3. fltr-specific: whether to theme with `classic` (Material look) or `modern` — a design decision, not a research question.
