...
The **Liveness Endpoint** (`/healthz`) is a **Reserved Path**: it is answered by fltr itself and never inspected, matched against Block Rules, or forwarded. Its responses carry `Cache-Control: no-store`, and the `405` sets `Allow: GET, HEAD`.
